 ...
 *
 *
 *Copyright(C) 2024 www.GuavyTechSolutions.com. All Rights Reserved.
 * @license http://www.gnu.org/licenses/gpl-3.0.html GNU/GPL
 * Author: GuavyTech Solutions
 * Website: http://www.GuavyTechSolutions.com
 * Forum: http://www.GuavyTechSolutions.com/forum/
 */
...
jQuery.fn.wowSlider = function (F) {
  ...
  N("<div>")
    ...
    .append(a.find("li:first img:first").clone().css({ width: "100%" }))
    .prependTo(a);
  S.css({
    position: "absolute",
    top: 0,
  ...
  if (
    (/MSIE/.test(A) && parseInt(/MSIE\s+([\d\.]+)/.exec(A)[1], 10) < 8) ||
    /Safari/.test(A)
  ) {
  ...
  N(al)
    .css({ zIndex: 11, right: "5px", bottom: "2px", display: "none" })
    .appendTo(T);
  ...
  if (W) {
    W.css({
      ...
      .attr({ href: "http://" + d.toLowerCase() })
      .html(d)
      .bind("contextmenu", function (c) {
        return false;
      })
      .show()
      .appendTo(al || document.body)
      .attr("target", "_blank");
  }
  if (F.controls) {
    ...
    if (U.length) {
        ...
        for (var aE = 0; aE < 2; aE++) {
          ...
          if (aA < 0) {
            var aB,
              aC,
              aG =
                (aF[aE ? "pageX" : "pageY"] - U.offset()[aE ? "left" : "top"]) /
                aI;
            if (ap == aG) {
              return;
            }
            ...
            if (Y > 0) {
              ...
              aB = aG < 0.5 ? 0 : aA - 1;
              aC =
                (Y * Math.abs(au.position()[aE ? "left" : "top"] - aB)) /
                (Math.abs(aG - 0.5) - aH);
            } else {
      ...
      ac(
        ...
        function (aC, aB, aA) {
          au.css(
            "left",
            Math.min(Math.max(aq - aB, U.width() - au.width()), 0)
          );
          au.css(
            "top",
            Math.min(Math.max(ar - aA, U.height() - au.height()), 0)
          );
          aC.preventDefault();
          return false;
        },
      ...
      if (an.length) {
        ...
        function at(aC) {
          ...
          var aD = {
            ...
            "margin-bottom":
              -aw.get(aC).offsetTop + aw.get(aw.length - 1).offsetTop + "px",
          };
          var aB = an.get(aC);
          var aA = {
            left:
              -aB.offsetLeft +
              (N(aB).outerWidth(true) - N(aB).outerWidth()) / 2,
          };
          if (c < 0) {
            ao.css(aD);
            f.css(aA);
    ...
    N("A", U).each(function (ao) {
      if (ao == c) {
        ...
        if (!O) {
          ...
          f.stop(true).animate({
            left: -Math.max(
              Math.min(an.left, -ap.left),
              an.left + am.width() - U.width()
            ),
            top: -Math.max(
              Math.min(an.top, 0),
              an.top + am.height() - U.height()
            ),
          });
        }
      } else {
  ...
  var V = {
    ...
    array: function (am, c, f) {
      p(
        c,
        am[Math.floor(Math.random() * am.length)],
        0.5,
        "easeOutElastic1",
        f
      );
  ...
  function G(f) {
    ...
    c.stop(1, 1)
      .stop(1, 1)
      .fadeOut(F.captionDuration / 3, function () {
        if (an || am) {
          c.html(
            (an ? "<span>" + an + "</span>" : "") +
              (am ? "<div>" + am + "</div>" : "")
  ...
  function p(ao, at, c, aq, am) {
    ...
    function ap(av) {
      ...
      N(an[av])
        ...
        .css({ opacity: 0 })
        .animate({ opacity: aw }, am, "easeOutCirc")
        .animate(
          { top: 0, left: 0 },
  ...
  function Z(ar, av) {
    ...
    var c =
      av.distance || (an == "top" ? ar.outerHeight(true) : ar.outerWidth(true));
  ...
  if (M) {
    if (window.Audio && M.canPlayType && M.canPlayType("audio/mp3")) {
      ...
      swfobject.createSWF(
        { data: X + "player_mp3_js.swf", width: "1", height: "1" },
        {
